...
- Your parent handles deployment via `POST /projects/:id/deploy`
...
```bash
curl -s -X POST http://127.0.0.1:4100/projects/43f795feb1fd58133b05ac91/deploy \
...
- `POST http://127.0.0.1:4100/agents/builder-f371ac2d/report` -- report your own completion or progress.
- `POST http://127.0.0.1:4100/agents/project-43f795feb1fd58133b05ac91/message` -- message your parent with questions, blockers, or status updates.
...
> Auto-created 2026-02-24
> Agent: project-43f795feb1fd58133b05ac91 (manager)
...
- manager (project-43f795feb1fd58133b05ac91): active
...
---
## Agent Identity
- **Agent ID:** `builder-f371ac2d`
- **Role:** builder
- **Project:** `43f795feb1fd58133b05ac91`
- **Parent:** `project-43f795feb1fd58133b05ac91`
- **Node:** 12D3KooWFR7AkUvFTA8AmYW3rvbrzpKS3N7jPAw8wmqn3GbP6ov3
- **Workspace:** C:\Users\jaira\.pando\agents\builder-f371ac2d\workspace
...
```bash
curl -X POST http://127.0.0.1:4100/agents/project-43f795feb1fd58133b05ac91/message \
  -H "Authorization: Bearer fa7fc899c746851a59a8db73672dd4713905ac0cd24d5dd9f3443c84eadb0646" \
  -H "Content-Type: application/json" \
...
curl -X POST http://127.0.0.1:4100/agents/spawn \
  ...
  -d '{
    "role": "builder",
    "parentId": "builder-f371ac2d",
    "projectId": "43f795feb1fd58133b05ac91",
...
### Check Agent Status
```bash
curl http://127.0.0.1:4100/agents/builder-f371ac2d/status
```
### Deploy (Phase 70 — ONE CALL)
After building web content, deploy via the unified endpoint:
```bash
curl -s -X POST http://127.0.0.1:4100/projects/43f795feb1fd58133b05ac91/deploy -H 'Content-Type: application/json' -H 'Authorization: Bearer fa7fc899c746851a59a8db73672dd4713905ac0cd24d5dd9f3443c84eadb0646' -d '{"workspaceDir": "C:\Users\jaira\.pando\agents\builder-f371ac2d\workspace"}'
```
This pushes to GitHub, then deploys to cloud (S3 for Tier 1, EC2 for Tier 2). Returns the live URL.
**ALWAYS deploy and share the URL. NEVER give local file paths. NEVER use /agents/:id/deploy (removed).**
**Do NOT create a new project — yours is already: 43f795feb1fd58133b05ac91**
